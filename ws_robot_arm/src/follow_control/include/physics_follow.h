...
#define __PHYSICS_FOLLOW_H
    ...
    class motion_physical
    {
        ...
        private:
            ...
            void homing(ArmDef& Arm);
            void joints_state_publish(ArmDef& Arm, string robot_ref);
            double vel_pid_realize(_pid *pid, int actual_val);
            int16_t SetPositionWithCurrent(ArmDef& Arm, uint8_t joint_num, int target_position, uint16_t current);
            void Follow_TrajFile(void);
            void Record_traj(void);
