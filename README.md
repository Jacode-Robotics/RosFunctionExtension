...
Config_masted_dxl() sets the main arm to current mode after returning to zero, with a current of 0. config_slave_dxl() sets the six joints of the main arm to position mode and the gripper to current mode. The gripper current is converted from the position difference between the two grippers to the target speed and controlled by the speed loop PID:
```
...
Using the current position of the main motor as the target position of the secondary motor, read and write at a certain frequency to achieve synchronous motion. During the reading and writing process, the previous trajectory can be recorded and reproduced:
```
// Read motor status
...
config_master_dxl()在主机械臂回到零点后将其设置为电流模式，电流为0，config_slave_dxl()则将从机械臂的主臂六个关节为位置模式，夹爪为电流模式，夹爪电流由两个机械臂夹爪位置差转化为目标速度并由速度环pid控制：
```
// Configure the motor of the slave robotic arm and return it to zero position
void motion_physical::config_slave_dxl(ArmDef& Arm)
{
...
将主电机的当前位置作为从电机的目标位置，按照一定频率读写，达到同步运动的目的,在读写过程中将可以记录和复现上一轮轨迹
```
// Read motor status
void motion_physical::statusRead(void)
{
